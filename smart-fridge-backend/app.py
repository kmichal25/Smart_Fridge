...
def add_product():
    ...
    # Walidacje
    # Nazwa nie może być pusta, nie może być liczbą, i musi zawierać litery
    if not name or not isinstance(name, str) or not re.search('[a-zA-Z]', name):
        return jsonify({'error': 'Niepoprawna nazwa produktu.'}), 400

    # Walidacja ilości
    if not isinstance(amount, (int, float)) or amount < 0:
        return jsonify({'error': 'Ilość musi być nieujemną liczbą.'}), 400
